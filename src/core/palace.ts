...
export interface Room {
  ...
  description: string;
  closet_summary: string | null;  // LLM-generated summary
  memory_count: number;
...
export interface Tunnel {
  ...
  from_wing: string;
  to_wing: string;
  room_name: string;      // same room name bridges the wings
  description: string;
  memory_count: number;
}
...
export class Palace {
  ...
  deleteWing(name: string): { deleted: boolean; rooms_removed: number; memories_invalidated: number } {
    ...
    const wingFile = path.join(this.palaceDir, `wing-${name}.json`);
    if (fs.existsSync(wingFile)) fs.unlinkSync(wingFile);
  ...
  listWings(): Wing[] {
    const wings: Wing[] = [];
    if (!fs.existsSync(this.palaceDir)) return wings;

    for (const file of fs.readdirSync(this.palaceDir)) {
      if (!file.startsWith('wing-') || !file.endsWith('.json')) continue;
      try {
  ...
  createRoom(wingName: string, roomName: string, description: string = ''): Room {
    ...
    if (!wing) throw new Error(`Wing '${wingName}' does not exist`);
  ...
  listRooms(wingName: string): Room[] {
    const rooms: Room[] = [];
    if (!fs.existsSync(this.palaceDir)) return rooms;

    for (const file of fs.readdirSync(this.palaceDir)) {
      if (!file.startsWith(`room-${wingName}-`) || !file.endsWith('.json')) continue;
      try {
        const data = JSON.parse(fs.readFileSync(path.join(this.palaceDir, file), 'utf-8'));
  ...
  getDrawers(wingName: string, roomName?: string, hall?: HallType): Drawer[] {
    ...
    return drawerIds
      .map(id => {
        const rows = (this.kg as any).db?.prepare('SELECT * FROM drawers WHERE drawer_id = ?').get(id) as Drawer | undefined;
        return rows;
      })
      .filter(Boolean) as Drawer[];
  }
  ...
  getClosets(wingName: string, roomName: string): Closet[] {
    ...
    if (!fs.existsSync(this.palaceDir)) return closets;
  ...
  findTunnels(wingName: string): Tunnel[] {
    ...
    if (!fs.existsSync(this.palaceDir)) return tunnels;

    for (const file of fs.readdirSync(this.palaceDir)) {
      if (!file.startsWith('tunnel-') || !file.endsWith('.json')) continue;
      try {
        const data = JSON.parse(fs.readFileSync(path.join(this.palaceDir, file), 'utf-8'));
   ...
   */
  findPotentialTunnels(): { roomName: string; wings: string[] }[] {
    const wingRooms = new Map<string, Set<string>>();
  ...
  walk(wingName?: string, hall?: HallType): {
    ...
    const wings = wingName ? [this.getWing(wingName)].filter(Boolean) as Wing[] : this.listWings();
    const rooms: Room[] = [];
    const closets: Closet[] = [];
    let totalMemories = 0;
  ...
  private inferMemoryType(hall: HallType, content: string): Memory['memory_type'] {
  ...
  overview(): string {
    ...
    for (const wing of wings) {
      ...
      if (wing.description) output += `${wing.description}\n`;
