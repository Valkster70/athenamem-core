...
const athenamem = definePluginEntry({
    ...
    register(api) {
        ...
        const expandPath = (p) => p && p.startsWith("~") ? path.join(homedir(), p.slice(1)) : p;
        ...
        api.registerHook("after_tool_call", async (event) => {
            if (Boolean(cfg?.auto_wal ?? true)) {
                try {
                    const c = getContext();
                    c.wal.checkpoint({ session_state: `tool:${event.toolCallId}` });
                }
                catch {
            ...
            if (Boolean(cfg?.compact_on_flush ?? true)) {
                try {
                    ...
                    if (oldMemories.length >= 3) {
                        await c.compaction.compact(oldMemories.slice(0, 10).map(m => m.id), 1, "athena", "memory-stack");
                    }
                }
                catch {
        ...
        api.registerTool({
            ...
            async execute() {
                ...
                return { content: [{ type: "text", text: JSON.stringify(wings, null, 2) }] };
            },
        });
        api.registerTool({
        ...
        api.registerTool({
            ...
            async execute(_, params) {
                const results = await toolSearch(String(params.query), params.wing != null ? String(params.wing) : undefined, params.room != null ? String(params.room) : undefined, params.limit != null ? Number(params.limit) : 20);
        ...
        api.registerTool({
            ...
            async execute(_, params) {
                const result = await toolKgQuery(params.entityId != null ? String(params.entityId) : undefined, params.asOf != null ? Number(params.asOf) : undefined);
                return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
            },
        });
        api.registerTool({
        ...
        api.registerTool({
            ...
            async execute(_, params) {
                const result = await toolResolveConflict(String(params.memoryId), String(params.resolution));
                return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
            },
        });
        api.registerTool({
            ...
            async execute(_, params) {
                const result = await toolDiaryWrite(String(params.agentName), String(params.entryType), String(params.content));
                return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
            },
        });
        api.registerTool({
            ...
            async execute(_, params) {
                const result = await toolDiaryRead(String(params.agentName), params.limit != null ? Number(params.limit) : 10);
                return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
            },
        });
        api.registerTool({
        ...
        api.registerTool({
            ...
            async execute(_, params) {
                ...
                const result = await toolCreateWing(String(params.wingName), desc);
                return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
            },
        });
        // Create Room Tool
