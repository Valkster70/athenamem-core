 ...
 */
export function extractFacts(text, source, timestamp) {
    const facts = [];
...
export class ContradictionDetector {
    ...
    check(facts) {
        ...
        for (const fact of facts) {
            ...
            for (const existing of existingRelations) {
                if (existing.object_id !== objectEntity.id)
                    continue;
    ...
    inferEntityType(name) {
        ...
        if (['team', 'we', 'everyone', 'they'].includes(lower))
            return 'agent';
...
// ─── Integration helper ────────────────────────────────────────────────────────
export function checkAndFlagContradictions(kg, memory, text) {
