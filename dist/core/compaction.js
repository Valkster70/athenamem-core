...
export class CompactionEngine {
    ...
    nodes = new Map();
    llm;
    constructor(kg, compactionDir, llm) {
        this.kg = kg;
    ...
    async compact(sourceIds, level, module, section) {
        ...
        for (const fact of facts) {
            const entityType = this.inferEntityType(fact.predicate);
            const subject = this.kg.addEntity(fact.subject, entityType);
            const object = this.kg.addEntity(fact.object, entityType);
            this.kg.addRelation(subject.id, fact.predicate, object.id, fact.confidence, sourceIds[0]);
        }
        const node = {
            id: genId(),
            level,
    ...
    expand(nodeId, depth = 0, maxDepth = 10) {
        if (depth > maxDepth) {
    ...
    targetTokens(sourceTokens, level) {
        const ratios = { 1: 0.7, 2: 0.2, 3: 0.05 };
        return Math.max(50, Math.round(sourceTokens * (ratios[level] ?? 0.2)));
    }
    buildCompactionPrompt(content, level, sourceCount) {
...
export class RuleBasedCompiler {
    async compile(prompt, maxTokens) {
